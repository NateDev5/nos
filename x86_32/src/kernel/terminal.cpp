...
namespace Kernel::Terminal {
    ...
    void handle_keypress (IN Drivers::Keyboard::KeypressInfo keypress) {
        if(keypress.flags & Drivers::Keyboard::KEYDOWN && keypress.scancode != SCANCODE_INVALID) {
            ...
            if(keypress.keycode == KEYCODE_BACKSPACE) {
                if(terminal_buffer_pos == 0) return;
                ...
                terminal_buffer_pos = Library::remove_at(terminal_buffer, TERMINAL_BUFFER_SIZE, terminal_buffer_pos, currentCursorPos - 1);
    ...
    void display_character (IN int8 _char) {
        ...
        if(terminal_buffer_pos > TERMINAL_BUFFER_SIZE) return;
