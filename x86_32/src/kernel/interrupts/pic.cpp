...
void init() {
    ...
    // ICW2
    outb(PIC1_DATA,
         EXTERNAL_INTERRUPTS_START); // move the offset to the start of the
                                     // external interrupts because if not it
                                     // will overwrite reserved interrupts
    outb(PIC2_DATA,
         EXTERNAL_INTERRUPTS_START + 8); // because there are 8 input lines
